...
const PresentationView = () => {
  ...
  if (loading) return <div className="text-center mt-5">Loading presentation...</div>;
  if (error) return <div className="alert alert-danger mt-5 m-4">{error}</div>;

  return (
    <div className="presentation-container">
      <header className="presentation-header">
        <h1 className="presentation-title">{data.name || "Contribution Presentation"}</h1>
        ...
        {data.contributions.map((item) => (
          <div key={item.id} className="timeline-card card mb-4 shadow-sm">
            <div className="card-body">
              ...
              {/* Categories - Matching Timeline logic */}
              {item.categories && (
                <div className="mb-2">
              ...
              <div className="mt-3">
                ...
                <h6><strong>Why It Mattered:</strong></h6>
                <p className="card-text text-muted">{item.why_it_mattered}</p>

                <h6><strong>Outcome & Impact:</strong></h6>
                <p className="card-text text-muted">{item.outcome_impact}</p>
              </div>

              {/* NEW: Evidence Links Section */}
              {item.evidence_links && item.evidence_links.length > 0 && (
                <div className="mt-3 pt-3 border-top">
                  <h6 className="text-dark small uppercase"><strong>Evidence Links:</strong></h6>
              ...
              {/* NEW: Attached Files Section (Metadata Only) */}
              {item.files && item.files.length > 0 && (
                <div className="mt-2">
                  <h6 className="text-dark small uppercase"><strong>Attached Evidence:</strong></h6>
                  <div className="d-flex flex-wrap">
                    {item.files.map((file, idx) => (
                      <span key={idx} className="badge border text-secondary me-2 mb-2 p-2">
                        📄 {file.file_name}
                      </span>
                    ))}
                  </div>
      ...
      <div className="text-center mt-5 mb-5 d-print-none">
        <button className="btn btn-primary px-4 shadow-sm" onClick={() => window.print()}>
