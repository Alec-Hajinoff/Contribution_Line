...
function AddContribution() {
  ...
  const [form, setForm] = useState({
    ...
    categories: [],
    contribution_date: "",
    evidence_links: [{ url: "", label: "" }], // Starts with one empty field
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append categories as array for PHP
      form.categories.forEach(cat => formData.append("categories[]", cat));
      
      // Append links as stringified JSON for easy backend parsing
      ...
      <div className="card-body">
        ...
        {isOpen && (
          <form onSubmit={handleSubmit} className="form-container">
            {/* ... existing fields: title, what_happened, why_it_mattered, outcome_impact ... */}
            ...
            <div className="mb-2">
              ...
              <textarea name="what_happened" className="form-control" value={form.what_happened} onChange={handleChange} rows={3} required />
            </div>

            <div className="mb-2">
              <label className="form-label">Why it mattered?</label>
              <textarea name="why_it_mattered" className="form-control" value={form.why_it_mattered} onChange={handleChange} rows={3} required />
            </div>

            <div className="mb-2">
              <label className="form-label">Outcome / impact</label>
              <textarea name="outcome_impact" className="form-control" value={form.outcome_impact} onChange={handleChange} rows={3} required />
            </div>

            <div className="mb-2">
              <label className="form-label">Category</label>
              <select name="categories" multiple className="form-select" value={form.categories} onChange={handleCategoriesChange}>
              ...
              {form.evidence_links.map((link, index) => (
                <div key={index} className="input-group mb-2">
                  ...
                  {form.evidence_links.length > 1 && (
                    <button type="button" className="btn btn-outline-danger" onClick={() => removeLinkField(index)}>×</button>
                  )}
                </div>
              ))}
              <button type="button" className="btn btn-sm btn-outline-secondary" onClick={addLinkField}>+ Add another link</button>
            </div>

            {/* File Upload Section */}
            <div className="mb-3">
              <label className="form-label">Support File (PDF, JPG, PNG - Max 10MB)</label>
              <input
                type="file"
                className="form-control"
            ...
            <div className="mb-3">
              ...
              <input type="date" name="contribution_date" className="form-control" value={form.contribution_date} onChange={handleChange} required />
            </div>

            <div className="d-flex align-items-center">
              <button type="submit" className="btn btn-primary me-2">Save</button>
              {status === "saving" && <small>Saving...</small>}
              {status === "success" && <small className="text-success"> Saved successfully.</small>}
